...
contract LottoNFT is ERC721A, Ownable {
    ...
    uint256 cost = 10;
    string baseURI =
        "https://ipfs.io/ipfs/QmYGgEFqTRkWvNZ6u7gfk9HDdh55bQAbYVyc16TF1zX658/";
    IERC20 coin;
    ILotto lotto;
    ...
    function mint() external {
        require(
            coin.allowance(msg.sender, address(this)) >= cost,
            "Sender has not given contract enough allowance"
        );
        require(
            coin.balanceOf(msg.sender) >= cost,
            "Sender does not have enough LottoCoin balance"
        );
